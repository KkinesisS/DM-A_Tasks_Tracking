    ...
    teams.forEach(team => {
        ...
        teamTasks.forEach(task => {
            ...
            row.innerHTML = `
                ...
                <td><span>${task.currentStatus === 'Completed' && task.completedDate ? `<strong style="color: #10b981;">Done ${task.completedDate}</strong>` : task.rtsDate}</span></td>
...
function renderWeeklySummaryTab() {
    ...
    const rtsTableBody = document.getElementById('summaryRtsTableBody');
      const rtsTable = rtsTableBody.closest('table');
      // Clear existing tbodys (including the default one)
      rtsTable.querySelectorAll('tbody').forEach(tbody => tbody.remove());
    ...
    if (sortedTasks.length === 0) {
        const emptyBody = document.createElement('tbody');
          emptyBody.innerHTML = '<tr><td colspan="8" style="text-align: center; color: var(--text-muted); padding: 1.5rem;">No tasks recorded for this period.</td></tr>';
          rtsTable.appendChild(emptyBody);
    } else {
