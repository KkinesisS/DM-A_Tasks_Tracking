const fs = require('fs');
const file = 'c:/Users/tg50700/OneDrive - THAI AIRWAYS INTERNATIONAL PUBLIC CO.,LTD/Desktop/DM-A Task Tracking Deployment/index.html';
let content = fs.readFileSync(file, 'utf8');

const target = `                    <!-- Notification Panel -->
...
const replacement = `                    <!-- Notification Panel -->
...
if (normalize(content).includes(normalize(target))) {
    ...
    const newContent = normalize(content).replace(new RegExp(escapedTarget, 'g'), normalize(replacement));
