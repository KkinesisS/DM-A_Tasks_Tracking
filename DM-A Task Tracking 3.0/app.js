...
function setupEventListeners() {
    ...
    const teamSel = document.getElementById('summaryTeamSelector');
    if (startSel && endSel) {
...
function renderComments(task) {
    ...
    if (updates.length === 0) {
        ...
        updates.forEach((update, idx) => {
            ...
            scrollContent.appendChild(createCommentDOM(update, isLatest));
        });
        updatesList.appendChild(scrollContent);
    }
    
    // Scroll both lists to the bottom
...
function exportToExcel() {
    ...
    if (activeTab === 'summary') {
        ...
        const teamSelector = document.getElementById('summaryTeamSelector');
        const selectedTeam = teamSelector ? teamSelector.value : 'All';
        
        // Filter by selected team exactly like the Weekly Summary tab
        tasksToExport = tasks.filter(t => {
            if (selectedTeam === 'All') return true;
            return (t.assignedTeam || '').split(',').map(s => s.trim()).includes(selectedTeam);
        });
    ...
    const rows = tasksToExport.map(task => {
        ...
        const updatesText = sortedUpdates.map((u, index) => {
            ...
            const vNum = `v${index + 1}.0`;
            return `${vNum} [${dateLabel}] ${u.author}: ${u.text}`;
        }).join('\n');
...
function renderWeeklySummaryTab() {
    ...
    // Update report period display label
    ...
    if (summaryPeriodDisplay) {
        ...
        summaryPeriodDisplay.textContent = `Report Period: ${startFormatted} - ${endFormatted}`;
    }

    // Link all tasks from the board to the weekly report
    const weeklyTasks = tasks;

    // Retrieve the selected team (if any)
    const teamSelector = document.getElementById('summaryTeamSelector');
    const selectedTeam = teamSelector ? teamSelector.value : 'All';

    // Filter tasks by selected team — supports multi-team (comma-separated)
    ...
    const teamsToRender = selectedTeam === 'All' ? teams : [selectedTeam];
    ...
    teamsToRender.forEach(teamName => {
        ...
        if (completedTasks.length > 0) {
            const details = completedTasks
                .map(t => `${t.aircraftReg} (${t.aircraftType || 'A/C'}): ${truncateSummaryString(t.taskDescription, 80)}`)
                .join(' / ');
            narrativeHtml += `
                <p style="margin:0; font-size:0.88rem; line-height:1.6;">
                    <span style="display:inline-block; font-size:0.7rem; font-weight:700; letter-spacing:0.05em; text-transform:uppercase; color:#059669; background:rgba(5,150,105,0.1); border-radius:4px; padding:0.1rem 0.45rem; margin-right:0.4rem; vertical-align:middle;">Successfully Completed</span>
                    ${details}.
                </p>`;
        }
        ...
        if (aogActive.length > 0) {
            const details = aogActive
                .map(t => `${t.aircraftReg} (${t.aircraftType || 'A/C'}): ${truncateSummaryString(t.taskDescription, 80)}`)
                .join(' / ');
            narrativeHtml += `
                <p style="margin:0; font-size:0.88rem; line-height:1.6;">
                    <span style="display:inline-block; font-size:0.7rem; font-weight:700; letter-spacing:0.05em; text-transform:uppercase; color:var(--priority-aog); background:rgba(220,38,38,0.1); border-radius:4px; padding:0.1rem 0.45rem; margin-right:0.4rem; vertical-align:middle;">Critical / AOG</span>
                    Managing ${aogActive.length} active AOG defect(s) — ${details}.
                </p>`;
        }
        ...
        if (normalActive.length > 0) {
            const details = normalActive
                .map(t => `${t.aircraftReg} (${t.aircraftType || 'A/C'})`)
                .join(', ');
            const statusSummary = [...new Set(normalActive.map(t => t.currentStatus))].join(' / ');
            narrativeHtml += `
                <p style="margin:0; font-size:0.88rem; line-height:1.6;">
                    <span style="display:inline-block; font-size:0.7rem; font-weight:700; letter-spacing:0.05em; text-transform:uppercase; color:#d97706; background:rgba(217,119,6,0.1); border-radius:4px; padding:0.1rem 0.45rem; margin-right:0.4rem; vertical-align:middle;">${statusSummary}</span>
                    Currently progressing ${normalActive.length} active task(s) on ${details}.
                </p>`;
        }
    ...
    if (sortedTasks.length === 0) {
        rtsTableBody.innerHTML = '<tr><td colspan="9" style="text-align: center; color: var(--text-muted); padding: 1.5rem;">No tasks recorded for this period.</td></tr>';
    } else {
        sortedTasks.forEach(t => {
            ...
            const toggleBtn = updates.length > 0
                ? `<button onclick="event.stopPropagation(); toggleWeeklyLogRow('${rowId}', this)" style="background:none;border:none;cursor:pointer;padding:4px;color:var(--text-muted);display:flex;align-items:center;justify-content:center;gap:4px;font-size:0.75rem;font-weight:600;transition:color 0.2s;" title="Toggle status log">
                ...
                : `<span style="color:var(--border-color);font-size:0.75rem;display:flex;align-items:center;justify-content:center;" title="No updates">—</span>`;
            ...
            row.innerHTML = `
                ...
                <td style="text-align:center;vertical-align:middle;">${toggleBtn}</td>
            `;
            rtsTableBody.appendChild(row);

            // Print-only log row (hidden on screen, visible when printing)
            if (updates.length > 0) {
                const sortedUpdates = [...updates].sort((a, b) => new Date(a.timestamp) - new Date(b.timestamp));
                const logRow = document.createElement('tr');
                logRow.id = rowId;
                logRow.className = 'wk-print-log-row';
                logRow.innerHTML = `
                    <td colspan="9" style="padding:0;border-top:none;">
                        <div style="padding:0.65rem 1rem 0.65rem 2rem;background:var(--bg-card);border-bottom:1px solid var(--border-color);border-left:3px solid var(--team-avionics);">
                            <div style="font-size:0.68rem;font-weight:700;letter-spacing:0.06em;text-transform:uppercase;color:var(--text-muted);margin-bottom:0.55rem;">Status Updates &amp; Log (${updates.length})</div>
                            <div style="position:relative;padding-left:1.5rem;">
                                <div style="position:absolute;left:0.35rem;top:0;bottom:0;width:1.5px;background:var(--border-color);"></div>
                                ${sortedUpdates.map((u, i) => {
                                    ...
                                    const vNum = `v${sortedUpdates.length - i}.0`;
                                    return `
                                        <div style="display:flex;gap:0.6rem;margin-bottom:0.6rem;align-items:flex-start;">
                                            <span style="flex-shrink:0;min-width:2.5rem;text-align:center;font-size:0.65rem;font-weight:700;color:var(--text-muted);background:var(--bg-secondary);border:1px solid var(--border-color);border-radius:10px;padding:0.1rem 0.3rem;margin-top:0.1rem;">${vNum}</span>
                                            <div style="flex:1;">
                                                <div style="font-size:0.8rem;font-weight:500;color:var(--text-primary);line-height:1.45;">${escapeHTMLApp(u.text)}</div>
                                                <div style="font-size:0.72rem;color:var(--text-muted);margin-top:0.15rem;">${u.author} &middot; ${dateLabel} ${relLabel}</div>
                                            </div>
                                        </div>`;
                                }).join('')}
                            </div>
    ...
    teamFyis.forEach(item => {
        ...
        card.innerHTML = `
            <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 0.75rem;">
                <div style="display: flex; flex-wrap: wrap; align-items: center; gap: 0.5rem; padding-right: 2rem;">
                ...
                <button type="button" onclick="deleteFYIItem('${item.id}')" title="Delete FYI" style="position: absolute; top: 1.25rem; right: 1.25rem; background: none; border: none; cursor: pointer; color: var(--priority-aog); opacity: 0.7; transition: opacity 0.2s;">
...
function showUpdateLogModal(taskId) {
    ...
    const sortedUpdates = [...updates].sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp)); // newest first
    ...
    const timelineHtml = sortedUpdates.length === 0
        ...
        : sortedUpdates.map((u, i) => {
            let dateLabel = '', timeLabel = '', relLabel = '';
            try {
                const d = new Date(u.timestamp);
                const months = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
                const day = String(d.getDate()).padStart(2,'0');
                ...
                const hr = String(d.getHours()).padStart(2,'0');
                const mn = String(d.getMinutes()).padStart(2,'0');
                dateLabel = `${day} ${mon}, ${yr}`;
                timeLabel = `${hr}:${mn}`;
                const diffMs = Date.now() - d.getTime();
                const diffH = Math.floor(diffMs / 3600000);
                const diffD = Math.floor(diffMs / 86400000);
                if (diffD === 0 && diffH === 0) relLabel = 'Just now';
                else if (diffD === 0) relLabel = `${diffH} hour${diffH !== 1 ? 's' : ''} ago`;
                else if (diffD === 1) relLabel = 'Yesterday';
                else if (diffD < 30) relLabel = `${diffD} days ago`;
                else relLabel = dateLabel;
            } catch(e) {}

            const vNum = `v${sortedUpdates.length - i}.0`;
            const isLatest = i === 0;
            const dotColor = isLatest ? 'var(--team-avionics)' : 'var(--border-color)';
            const vBg = isLatest ? 'var(--team-avionics)' : 'var(--bg-secondary)';
            const vTxtColor = isLatest ? '#fff' : 'var(--text-muted)';

            return `
            <div style="display:flex;gap:1rem;padding:1rem 0;border-bottom:1px solid var(--border-color);" class="update-log-entry">
                <!-- Version badge + timeline line -->
                <div style="display:flex;flex-direction:column;align-items:center;gap:0;flex-shrink:0;width:3rem;">
                    <span style="font-size:0.68rem;font-weight:800;background:${vBg};color:${vTxtColor};border-radius:20px;padding:0.18rem 0.4rem;border:1.5px solid ${dotColor};white-space:nowrap;">${vNum}</span>
                    ${i < sortedUpdates.length - 1 ? `<div style="flex:1;width:1.5px;background:var(--border-color);margin-top:4px;min-height:1.5rem;"></div>` : ''}
                </div>
                <!-- Content -->
                <div style="flex:1;padding-top:0.1rem;">
                    <div style="font-size:0.9rem;font-weight:500;color:var(--text-primary);line-height:1.5;margin-bottom:0.3rem;">${escapeHTMLApp(u.text)}</div>
                    <div style="font-size:0.77rem;color:var(--text-muted);display:flex;align-items:center;gap:0.4rem;flex-wrap:wrap;">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="width:11px;height:11px;flex-shrink:0;"><circle cx="12" cy="12" r="10"></circle><polyline points="12 6 12 12 16 14"></polyline></svg>
                        <strong style="color:var(--text-secondary);">${u.author}</strong>
                        <span>&middot;</span>
                        <span>${relLabel}</span>
