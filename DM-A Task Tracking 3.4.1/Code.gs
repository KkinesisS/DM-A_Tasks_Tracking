...
function initSheet() {
  ...
  const tasksHeaders = [
    ...
    'currentStatus', 'attachments', 'comments', 'isExternal', 'completedDate', 'topic'
  ];
  ...
  if (isNewTasks) {
    ...
    if (data.length > 0) {
      ...
      // Check if 'completedDate' column exists in headers, if not add it
      const updatedData2 = sheet.getDataRange().getValues();
      if (updatedData2.length > 0) {
        const updatedHeaders2 = updatedData2[0];
        if (updatedHeaders2.indexOf('completedDate') === -1) {
          sheet.getRange(1, updatedHeaders2.length + 1).setValue('completedDate');
        }
      }

      // Check if 'topic' column exists in headers, if not add it
      const updatedData3 = sheet.getDataRange().getValues();
      if (updatedData3.length > 0) {
        const updatedHeaders3 = updatedData3[0];
        if (updatedHeaders3.indexOf('topic') === -1) {
          sheet.getRange(1, updatedHeaders3.length + 1).setValue('topic');
        }
      }
    }
  }
